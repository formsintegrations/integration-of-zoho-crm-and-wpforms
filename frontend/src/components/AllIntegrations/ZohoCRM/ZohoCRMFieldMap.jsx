    ...
    >
      <div className="flx integ-fld-wrp">
        <select className="btcd-paper-inp mr-2" name="formField" value={field.formField || ''} onChange={(ev) => handleFieldMapping(ev, i, crmConf, setCrmConf, uploadFields, tab)}>
          <option value="">{__('Select Field', 'bitwpfzc')}</option>
          {
          ...
          {!uploadFields && <option value="custom">{__('Custom...', 'bitwpfzc')}</option>}
        </select>

        {field.formField === 'custom' && <MtInput onChange={e => handleCustomValue(e, i, crmConf, setCrmConf, tab)} label={__('Custom Value', 'bitwpfzc')} className="mr-2" type="text" value={field.customValue} placeholder={__('Custom Value', 'bitwpfzc')} />}
        ...
        <select className="btcd-paper-inp" disabled={!isNotRequired} name="zohoFormField" value={field.zohoFormField || ''} onChange={(ev) => handleFieldMapping(ev, i, crmConf, setCrmConf, uploadFields, tab)}>
          ...
          <option value="" disabled>{__('More fields on the Pro version', 'bitwpfzc')}</option>
        </select>
      </div>
