  ...
  const removeInteg = i => {
    ...
    bitsFetch({ formId: formID, id: tempIntegration.id }, 'integration/delete').then(response => {
      if (response && response.success) {
        ...
        setSnackbar({
          show: true,
          msg: `${__('Integration deletion failed Cause', 'bitwpfzc')}:${response.data}. ${__(
            'please try again',
            'bitwpfzc'
          )}`
        })
                  ...
                  <div role="button" className="btcd-inte-card mr-4 mt-3" key={`inte-${i + 3}`}>
                    ...
                    <div className="btcd-inte-atn txt-center">
                      <Link
                        ...
                        style={{ '--tooltip-txt': `'${__('Edit', 'bitwpfzc')}'` }}
                        type="button"
                      >
                        <EditIcn size="15   " />
                      </Link>
                      <button
                        className="btn btcd-btn-o-blue btcd-btn-sm mr-2 tooltip pos-rel"
                        style={{ '--tooltip-txt': `'${__('Delete', 'bitwpfzc')}'` }}
                      ...
                      {typeof integs.find(int => int.type === inte.type)?.info !== 'boolean' && (
                        <Link
                          ...
                          className="btn btcd-btn-o-blue btcd-btn-sm mr-2 tooltip pos-rel"
                          style={{ '--tooltip-txt': `'${__('Info', 'bitwpfzc')}'` }}
                          type="button"
                        >
                      ...
                      <Link
                        ...
                        style={{ '--tooltip-txt': `'${__('Log', 'bitwpfzc')}'` }}
                        type="button"
                      >
                        <TimeIcn size="15" />
                      </Link>
