...
function LogicBlock({ fieldVal, formFields, fields, delLogic, lgcGrpInd, lgcInd, subLgcInd, subSubLgcInd, value, addInlineLogic, changeLogic, logicValue, changeValue, changeFormField }) {
  ...
  return (
    <div className="flx pos-rel btcd-logic-blk">
      ...
      >
        <option value="">{__('Select Form Field', 'bitwpfzc')}</option>
      ...
      <MtSelect
        ...
        className="w-4"
      >
        <option value="">{__('Select One', 'bitwpfzc')}</option>
        <option value="equal">{__('Equal', 'bitwpfzc')}</option>
        <option value="not_equal">{__('Not Equal', 'bitwpfzc')}</option>
        <option value="null">{__('Is Null', 'bitwpfzc')}</option>
        <option value="not_null">{__('Is Not Null', 'bitwpfzc')}</option>
        {!type.match(/^(date|time|datetime|month|week)$/) && <option value="contain">{__('Contain', 'bitwpfzc')}</option>}
        {((fldType === 'select' && fields?.[fieldKey]?.mul) || fldType === 'check') && <option value="contain_all">{__('Contain All', 'bitwpfzc')}</option>}
        {!type.match(/^(date|time|datetime|month|week)$/) && <option value="not_contain">{__('Not Contain', 'bitwpfzc')}</option>}
        {type === 'number' && <option value="greater">{__('Greater Than', 'bitwpfzc')}</option>}
        {type === 'number' && <option value="less">{__('Less Than', 'bitwpfzc')}</option>}
        {type === 'number' && <option value="greater_or_equal">{__('Greater Than or Equal', 'bitwpfzc')}</option>}
        {type === 'number' && <option value="less_or_equal">{__('Less Than or Equal', 'bitwpfzc')}</option>}
        {!type.match(/^(color|url|password|email|date|time|datetime|month|week)$/) && <option value="start_with">{__('Start With', 'bitwpfzc')}</option>}
        {!type.match(/^(color|url|password|email|date|time|datetime|month|week)$/) && <option value="end_with">{__('End With', 'bitwpfzc')}</option>}
      </MtSelect>
