  ...
  return (
    <div className="pos-rel">
      ...
      <div className="d-flx flx-wrp">
        <TableCheckBox onChange={(e) => actionHandler(e, 'workflow')} checked={tab === 0 ? 'workflow' in crmConf.actions : 'workflow' in crmConf.relatedlists[tab - 1].actions} className="wdt-200 mt-4 mr-2" value="Workflow" title={__('Workflow', 'bitwpfzc')} subTitle={__('Trigger CRM workflows', 'bitwpfzc')} />
        <TableCheckBox onChange={() => setActionMdl({ show: 'attachment' })} checked={tab === 0 ? 'attachment' in crmConf.actions : 'attachment' in crmConf.relatedlists[tab - 1].actions} className="wdt-200 mt-4 mr-2" value="Attachment" title={__('Attachment', 'bitwpfzc')} subTitle={__('Add attachments or signatures from BitForm to CRM.', 'bitwpfzc')} />
        <TableCheckBox onChange={(e) => actionHandler(e, 'approval')} checked={tab === 0 ? 'approval' in crmConf.actions : 'approval' in crmConf.relatedlists[tab - 1].actions} className="wdt-200 mt-4 mr-2" value="Approval" title={__('Approval', 'bitwpfzc')} subTitle={__('Send entries to CRM approval list.', 'bitwpfzc')} />
        <TableCheckBox onChange={(e) => actionHandler(e, 'blueprint')} checked={tab === 0 ? 'blueprint' in crmConf.actions : 'blueprint' in crmConf.relatedlists[tab - 1].actions} className="wdt-200 mt-4 mr-2" value="Blueprint" title={__('Blueprint', 'bitwpfzc')} subTitle={__('Trigger CRM Blueprint', 'bitwpfzc')} />
        <TableCheckBox onChange={(e) => actionHandler(e, 'gclid')} checked={tab === 0 ? 'gclid' in crmConf.actions : 'gclid' in crmConf.relatedlists[tab - 1].actions} className="wdt-200 mt-4 mr-2" value="Capture_GCLID" title={__('Capture GCLID', 'bitwpfzc')} subTitle={__('Sends the click details of AdWords Ads to Zoho CRM.', 'bitwpfzc')} />
        <TitleModal action={openUpsertModal}>
          <TableCheckBox onChange={(e) => actionHandler(e, 'upsert')} checked={tab === 0 ? 'upsert' in crmConf.actions : 'upsert' in crmConf.relatedlists[tab - 1].actions} className="wdt-200 mt-4 mr-2" value="Upsert_Record" title={__('Upsert Records', 'bitwpfzc')} subTitle={__('The record is updated if it already exists else it is inserted as a new record.', 'bitwpfzc')} />
        </TitleModal>
        <TableCheckBox checked={tab === 0 ? 'assignment_rules' in crmConf.actions : 'assignment_rules' in crmConf.relatedlists[tab - 1].actions} className="wdt-200 mt-4 mr-2" value="Assignment_Rule" title={__('Assignment Rules', 'bitwpfzc')} subTitle={__('Trigger Assignment Rules in Zoho CRM.', 'bitwpfzc')} />
        <TableCheckBox onChange={() => setActionMdl({ show: 'tag_rec' })} checked={tab === 0 ? 'tag_rec' in crmConf.actions : 'tag_rec' in crmConf.relatedlists[tab - 1].actions} className="wdt-200 mt-4 mr-2" value="Tag_Records" title={__('Tag Records', 'bitwpfzc')} subTitle={__('Add a tag to records pushed to Zoho CRM.', 'bitwpfzc')} />
        <TableCheckBox checked={tab === 0 ? 'rec_owner' in crmConf.actions : 'rec_owner' in crmConf.relatedlists[tab - 1].actions} className="wdt-200 mt-4 mr-2" value="Record_Owner" title={__('Record Owner', 'bitwpfzc')} subTitle={__('Add a owner to records pushed to Zoho CRM.', 'bitwpfzc')} />
      </div>
      ...
      <ConfirmModal
        ...
        mainMdlCls="o-v"
        btnClass="blue"
        btnTxt={__('Ok', 'bitwpfzc')}
        show={actionMdl.show === 'attachment'}
        close={clsActionMdl}
        action={clsActionMdl}
        title={__('Select Attachment', 'bitwpfzc')}
      >
        <div className="btcd-hr mt-2" />
        <div className="mt-2">{__('Select file upload fields', 'bitwpfzc')}</div>
        <MultiSelect
      ...
      <ConfirmModal
        ...
        close={clsActionMdl}
        action={clsActionMdl}
        title={__('Assignment Rules', 'bitwpfzc')}
      >
        <div className="btcd-hr mt-2" />
        <div className="mt-2">{__('Assignment Rules', 'bitwpfzc')}</div>
        {isLoading
          ? (
            <Loader style={{
              display: 'flex',
          ...
          : (
            <div className="flx flx-between mt-2">
              ...
              >
                <option value="">{__('Select Assignment Rule', 'bitwpfzc')}</option>
              ...
              <button className="icn-btn sh-sm ml-2 mr-2 tooltip" style={{ '--tooltip-txt': `'${__('Refresh CRM Assignment Rules', 'bitwpfzc')}'` }} type="button" disabled={isLoading}>&#x21BB;</button>
            </div>
          )}
      </ConfirmModal>
      ...
      <ConfirmModal
        ...
        close={clsActionMdl}
        action={clsActionMdl}
        title={__('Tag Records', 'bitwpfzc')}
      >
        <div className="btcd-hr mt-2 mb-2" />
        <small>{__('Add a tag to records pushed to Zoho CRM', 'bitwpfzc')}</small>
        <div className="mt-2">{__('Tag Name', 'bitwpfzc')}</div>
        {isLoading
          ? (
            <Loader style={{
              display: 'flex',
          ...
          : (
            <div className="flx flx-between mt-2">
              <MultiSelect
                ...
                customValue
              />
              <button className="icn-btn sh-sm ml-2 mr-2 tooltip" style={{ '--tooltip-txt': `${__('Refresh CRM Tags', 'bitwpfzc')}'` }} type="button" disabled={isLoading}>&#x21BB;</button>
            </div>
          )}
      ...
      <ConfirmModal
        ...
        close={clsActionMdl}
        action={clsActionMdl}
        title={__('Record Owner', 'bitwpfzc')}
      >
        <div className="btcd-hr mt-2" />
        <div className="mt-2">{__('Owner Name', 'bitwpfzc')}</div>
        {isLoading
          ? (
            <Loader style={{
              display: 'flex',
          ...
          : (
            <div className="flx flx-between mt-2">
              ...
              >
                <option value="">{__('Select Owner', 'bitwpfzc')}</option>
              ...
              <button className="icn-btn sh-sm ml-2 mr-2 tooltip" style={{ '--tooltip-txt': `'${__('Refresh CRM Owners', 'bitwpfzc')}'` }} type="button" disabled={isLoading}>&#x21BB;</button>
            </div>
          )}
      </ConfirmModal>
      ...
      <Modal
        ...
        title={__('Upsert Record', 'bitwpfzc')}
      >
        <div className="o-a">
          {
            tab === 0
              ? crmConf?.actions?.upsert && (
                <>
                  <div className="font-w-m mt-2">{__('Upsert Using', 'bitwpfzc')}</div>
                  <small>{__('Arrange fields in order of preferance for upsertion', 'bitwpfzc')}</small>
