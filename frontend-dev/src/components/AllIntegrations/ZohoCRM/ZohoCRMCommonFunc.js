  ...
  bitsFetch(refreshModulesRequestParams, 'zcrm_refresh_modules')
    .then(result => {
      if (result && result.success) {
        ...
        setSnackbar({ show: true, msg: sprintf(__('Modules refresh failed Cause: %s. please try again', 'bitwpfzc'), result.data.data || result.data) })
      } else {
        setSnackbar({ show: true, msg: __('Modules refresh failed. please try again', 'bitwpfzc') })
      }
      setisLoading(false)
    })
  ...
  bitsFetch(refreshLayoutsRequestParams, 'zcrm_refresh_layouts')
    .then(result => {
      if (result && result.success) {
        ...
        setSnackbar({ show: true, msg: sprintf(__('Layouts refresh failed Cause: %s. please try again', 'bitwpfzc'), result.data.data || result.data) })
      } else {
        setSnackbar({ show: true, msg: __('Layouts refresh failed. please try again', 'bitwpfzc') })
      }
      setisLoading(false)
    })
    .catch(() => setisLoading(false))
...
export const handleAuthorize = (confTmp, setConf, setError, setisAuthorized, setisLoading, setSnackbar) => {
  if (!confTmp.dataCenter || !confTmp.clientId || !confTmp.clientSecret) {
    setError({
      dataCenter: !confTmp.dataCenter ? __('Data center cann\'t be empty', 'bitwpfzc') : '',
      clientId: !confTmp.clientId ? __('Client ID cann\'t be empty', 'bitwpfzc') : '',
      clientSecret: !confTmp.clientSecret ? __('Secret key cann\'t be empty', 'bitwpfzc') : '',
    })
    return
  }
  ...
  const popupURLCheckTimer = setInterval(() => {
    if (authWindow.closed) {
      ...
      if (!grantTokenResponse.code || grantTokenResponse.error || !grantTokenResponse || !isauthRedirectLocation) {
        ...
        setSnackbar({ show: true, msg: `${__('Authorization failed', 'bitwpfzc')} ${errorCause}. ${__('please try again', 'bitwpfzc')}` })
        setisLoading(false)
      } else {
  ...
  bitsFetch(tokenRequestParams, 'zcrm_generate_token')
    ...
    .then(result => {
      if (result && result.success) {
        ...
        setSnackbar({ show: true, msg: __('Authorized Successfully', 'bitwpfzc') })
      } else if ((result && result.data && result.data.data) || (!result.success && typeof result.data === 'string')) {
        setSnackbar({ show: true, msg: `${__('Authorization failed Cause:', 'bitwpfzc')}${result.data.data || result.data}. ${__('please try again', 'bitwpfzc')}` })
      } else {
        setSnackbar({ show: true, msg: __('Authorization failed. please try again', 'bitwpfzc') })
      }
      setisLoading(false)
    })
}
