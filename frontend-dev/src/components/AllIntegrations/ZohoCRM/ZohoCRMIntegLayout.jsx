  ...
  return (
    <>
      ...
      <select onChange={handleInput} name="module" value={crmConf.module} className="btcd-paper-inp w-7" disabled={tab}>
        ...
        <option value="" disabled>{__('More module on the Pro version', 'bitwpfzc')}</option>
      </select>
      {tab === 0 && <button onClick={() => refreshModules(formID, crmConf, setCrmConf, setisLoading, setSnackbar)} className="icn-btn sh-sm ml-2 mr-2 tooltip" style={{ '--tooltip-txt': `'${__('Refresh CRM Modules', 'bitwpfzc')}'` }} type="button" disabled={isLoading}>&#x21BB;</button>}
      <br />
      <div>
        <Tabs>
          <div className="flx mt-2">
            ...
            {crmConf.relatedlists.length < 3 && <button onClick={addNewRelatedTab} className="icn-btn sh-sm ml-2 mr-2 tooltip" style={{ '--tooltip-txt': `'${__('Add More Related List', 'bitwpfzc')}'` }} type="button">+</button>}
          </div>
