...
function Integrations() {
  ...
  useEffect(() => {
    ...
    bitsFetch(data, 'ff/get/form')
      .then(res => {
        ...
        setSnackbar({ ...{ show: true, msg: __('Failed to load form data', 'bitwpfzc') } })
      })
  }, [])
    ...
    bitsFetch({ formId: formID, id: tempIntegration.id }, 'integration/delete')
      .then(response => {
        if (response && response.success) {
          ...
          setIntegration([...newInteg])
          setSnackbar({ show: true, msg: `${__('Integration deletion failed Cause', 'bitwpfzc')}:${response.data}. ${__('please try again', 'bitwpfzc')}` })
        } else {
          newInteg.splice(i, 0, tempIntegration)
          setIntegration([...newInteg])
          setSnackbar({ show: true, msg: __('Integration deletion failed. please try again', 'bitwpfzc') })
        }
      })
  }
  ...
  const inteDelConf = i => {
    ...
    confMdl.body = __('Are you sure to delete this integration?', 'bitwpfzc')
    confMdl.btnClass = ''
    ...
    <div className="btcd-s-wrp">
      ...
      <Switch>
        <Route exact path={path}>
          <h2>{__('Integrations', 'bitwpfzc')}</h2>
            ...
            {integrations.map((inte, i) => (
              <div role="button" className="btcd-inte-card mr-4 mt-3" key={`inte-${i + 3}`}>
                ...
                <div className="btcd-inte-atn txt-center">
                  <Link to={`${allIntegURL}/edit/${i}`} className="btn btcd-btn-o-blue btcd-btn-sm mr-2 tooltip pos-rel" style={{ '--tooltip-txt': `'${__('Edit', 'bitwpfzc')}'` }} type="button">
                    <EditIcn size="15   " />
                  </Link>
                  <button className="btn btcd-btn-o-blue btcd-btn-sm mr-2 tooltip pos-rel" style={{ '--tooltip-txt': `'${__('Delete', 'bitwpfzc')}'` }} onClick={() => inteDelConf(i)} type="button">
                    <span className="btcd-icn icn-trash-2" />
                  </button>
                  {typeof (integs.find(int => int.type === inte.type)?.info) !== 'boolean' && (
                    <Link to={`${allIntegURL}/info/${i}`} className="btn btcd-btn-o-blue btcd-btn-sm mr-2 tooltip pos-rel" style={{ '--tooltip-txt': `'${__('Info', 'bitwpfzc')}'` }} type="button">
                      <span className="btcd-icn icn-information-outline" />
                    </Link>
                  )}
                  <Link to={`${allIntegURL}/log/${i}`} className="btn btcd-btn-o-blue btcd-btn-sm tooltip pos-rel" style={{ '--tooltip-txt': `'${__('Log', 'bitwpfzc')}'` }} type="button">
                    <TimeIcn size="15" />
                  </Link>
                </div>
