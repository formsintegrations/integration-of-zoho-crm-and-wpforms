...
function ActionBlock({ formFields, fields, action, lgcGrpInd, actionInd, setworkFlows, actionType }) {
  ...
  return (
    <div className="flx pos-rel btcd-logic-blk">
      ...
      >
        <option value="">{__('Select One', 'bitwpfzc')}</option>
        <option value="value">{__('Value', 'bitwpfzc')}</option>
        {actionType !== 'onsubmit' && <option value="disable">{__('Disable', 'bitwpfzc')}</option>}
        {actionType !== 'onsubmit' && <option value="readonly">{__('Readonly', 'bitwpfzc')}</option>}
        {actionType !== 'onsubmit' && <option value="enable">{__('Enable', 'bitwpfzc')}</option>}
        {actionType !== 'onsubmit' && <option value="hide">{__('Hide', 'bitwpfzc')}</option>}
        {actionType !== 'onsubmit' && <option value="show">{__('Show', 'bitwpfzc')}</option>}
      </MtSelect>
